...
var getGenesisBlock = () => {
    return new Block(0, "0", 1682839690, "Genesis-block", "6c4a72ec0869af8c084e7fac525c57d5", 0);
};
...
var calculateHash = (index, previousHash, timestamp, data, nonce) => {
    return CryptoJS.MD5(index + sumDigits(previousHash).toString(2)  + timestamp + data + nonce + previousHash.reverse).toString();
};
...
//функция определения самой длинной цепочки
...
var mineBlock = (blockData) => {
    ...
    var nonce = 0;
    var lastHashDigitsSum = 0;
    var lastHashDigitsSumBinary = 0;
    var nextTimestamp = new Date().getTime() / 1000;

    var nextHash = calculateHash(nextIndex, previousBlock.hash, nextTimestamp,
        blockData, nonce);

    lastHashDigitsSum = sumDigits(nextHash);
    lastHashDigitsSumBinary = lastHashDigitsSum.toString(2);


    while (true) {
        if ((lastHashDigitsSumBinary == lastHashDigitsSumBinary.split("").reverse().join("")) && nextHash.includes("2008")) {
            break;
        } else {
            nonce++;
    ...
    console.log(lastHashDigitsSumBinary)
